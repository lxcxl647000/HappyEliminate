...
export default class PanelRouter {
     ...
     */
    init(rootNode: Node, enableDebugLog: boolean) {
    ...
    load(panelConfig: PanelConfig, onCompleted: (error?: Error) => void) {
        ...
        if (panelCache) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试加载面板", "面板已加载成功");
            onCompleted(null);
            return;
        }
        this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试加载面板", "面板还不存在，将开始加载面板");
        ...
        this._loadBundlePrefab(panelConfig, null, (error: Error, prefab: Prefab) => {
            ...
            if (error) {
                // 处理面板实际加载失败的逻辑
                this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试加载面板", "加载面板失败", error);
                if (!panelCache) {
                    this._enabledLog && console.warn(PanelRouter.Tag, prefabPath, "尝试加载面板", "当前已经找不到面板记录，可能面板加载过程中触发销毁，导致加载结束后找不到对应面板记录");
                } else {
                    if (
                        ...
                        this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试加载面板", "面板状态异常，原则上不会出现这个状态", panelCache.state);
                    } else {
                        // 更新面板状态
                ...
                // 处理面板实际加载成功的逻辑
                this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试加载面板", "加载面板成功");
    ...
    show(option: ShowPanelOption) {
        ...
        let panelCache = this._panelCacheMap.get(prefabPath);
        if (!panelCache) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板还没有加载，不能展示");
            option.onShowed && option.onShowed(new Error("show error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.Loading) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板在加载中，不能展示");
            option.onShowed && option.onShowed(new Error("show error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.LoadFailure) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板加载失败，不能展示");
            option.onShowed && option.onShowed(new Error("show error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.LoadSuccess) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板已加载成功，将尝试展示面板");
            this._showPanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Showing) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板正在展示中");
            this._showPanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Showed) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板已展示");
            this._showPanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Hiding) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板在隐藏中，将停止隐藏动画，并立即开始展示");
            this._showPanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Hided) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试展示面板", "面板已隐藏，将重新展示");
            this._showPanel(option, panelCache);
            return;
        }
    }
    ...
    private _showPanel(option: ShowPanelOption, panelCache: PanelCache) {
        ...
        panelCache.node.getComponent(PanelComponent).show({
            data: option.data,
            onShowed: () => {
                this._enabledLog && console.log(PanelRouter.Tag, option.panel.prefabPath, "尝试展示面板", "面板已经展示完成");
                panelCache.state = PanelStateEnum.Showed;
    ...
    hide(option: HidePanelOption) {
        ...
        let panelCache = this._panelCacheMap.get(prefabPath);
        if (!panelCache) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板还没有加载，不能隐藏");
            option.onHided && option.onHided(new Error("hide error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.Loading) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板在加载中，不能隐藏");
            option.onHided && option.onHided(new Error("hide error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.LoadFailure) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板加载失败，不能隐藏");
            option.onHided && option.onHided(new Error("hide error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.LoadSuccess) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板已加载成功，但未展示，不能隐藏");
            option.onHided && option.onHided(new Error("hide error"));
            return;
        }
        if (panelCache.state === PanelStateEnum.Showing) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板在展示中（可能正在执行展示动画），将停止展示并立即隐藏");
            this._hidePanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Showed) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板已展示，将立即隐藏");
            this._hidePanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Hiding) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板正在隐藏中");
            this._hidePanel(option, panelCache);
            return;
        }
        if (panelCache.state === PanelStateEnum.Hided) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试隐藏面板", "面板已隐藏");
            this._hidePanel(option, panelCache);
            return;
        }
    }
    ...
    private _hidePanel(option: HidePanelOption, panelCache: PanelCache) {
        ...
        panelCache.node.getComponent(PanelComponent).hide({
            data: option.data,
            onHided: () => {
                this._enabledLog && console.log(PanelRouter.Tag, option.panel.prefabPath, "尝试隐藏面板", "面板隐藏成功");
                panelCache.state = PanelStateEnum.Hided;
    ...
    destroy(option: CommonPanelOption) {
        ...
        if (!panel) {
            this._enabledLog && console.error(PanelRouter.Tag, prefabPath, "尝试销毁面板", "当前面板还不存在（建议检查为什么此时触发销毁方法）");
            return;
        }
        if (panel.state === PanelStateEnum.Loading) {
            this._enabledLog && console.warn(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板在加载中时，将销毁面板资源（建议检查为什么此时触发了销毁方法）");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.LoadFailure) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板加载已经失败，将销毁面板资源");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.LoadSuccess) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板加载已经成功（但无后续操作），将销毁面板资源");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.Showing) {
            this._enabledLog && console.warn(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板在展示中（可能正在执行展示动画），将销毁面板资源（建议检查为什么此时触发了销毁方法）");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.Showed) {
            this._enabledLog && console.warn(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板已展示，将销毁面板资源（建议检查为什么此时触发了销毁方法）");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.Hiding) {
            this._enabledLog && console.warn(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板在隐藏中（可能正在执行隐藏动画），将销毁面板资源（建议检查为什么此时触发了销毁方法）");
            this._destroyPanel(panel, prefabPath);
            return;
        }
        if (panel.state === PanelStateEnum.Hided) {
            this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板已隐藏，将销毁面板资源");
            this._destroyPanel(panel, prefabPath);
            return;
        }
    }
    ...
    private _destroyPanel(panelCache: PanelCache, prefabPath: string) {
        ...
        this._enabledLog && console.log(PanelRouter.Tag, prefabPath, "尝试销毁面板", "面板销毁成功");
    }
