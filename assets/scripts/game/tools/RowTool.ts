 ...
 * 行消除
 */
export class RowTool implements ITool {
        ...
        if (data.swapBlock && data.swapBlock.tool && data.swapBlock.tool.getToolType() === ToolType.Col) {
            data.grid.rangeBlocks((c: Block, i: number, j: number) => {
                if (c.blockGridID.y === data.block.blockGridID.y
                    || c.blockGridID.y === data.block.blockGridID.y - 1
                    || c.blockGridID.y === data.block.blockGridID.y + 1) {
                ...
                if (c.blockGridID.x === data.swapBlock.blockGridID.x
                    || c.blockGridID.x === data.swapBlock.blockGridID.x - 1
                    || c.blockGridID.x === data.swapBlock.blockGridID.x + 1) {
                    if (c.blockGridID.y === data.block.blockGridID.y) {
